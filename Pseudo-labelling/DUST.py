...
def forward_pass(model:Wav2Vec2ForCTC, batch:Dict[str, torch.Tensor]):
    with torch.no_grad():
        logits = model(**{k: v.to(model.device) for k, v in batch.items()})
    return logits
...
def stack_batch(batch:Dict[str, torch.Tensor], monte_carlo:int):
    return {k: torch.stack([v for _ in range(monte_carlo)], dim=0) for k, v in batch.items()}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--w2v', help='path to pretrained model', default='wav2vec2-large-robust-ft-libri-960h')
    parser.add_argument('--proc', help='path to pretrained processer for w2v', default='wav2vec2-large-robust-ft-libri-960h_proc')
    parser.add_argument('--csv', help='path to csv file with data', default='fred.csv')
    parser.add_argument('--csv_out', help='path to csv file with predictions', default='fred_pred.csv')
    parser.add_argument('--audio_dir', help='path to audio directory', default='audio')
    parser.add_argument('--audio_i', help='index of audio file in csv', default=0)
    parser.add_argument('--batch_size', help='batch size', default=15)

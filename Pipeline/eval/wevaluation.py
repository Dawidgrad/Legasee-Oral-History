...
def score_name(name,
    ...
    gold_ts = _read_gold_txt(_gfolder, name)
    
    # Get system transcripts (potentially from multiple files)
    _mfolder = model_folder
...
def _read_gold_txt(gold_folder, name) -> List:
    """
    Reads a gold standard text file
    """
    with open(gold_folder+'/'+name+'.txt','r') as goldin:
        gold_ts = goldin.readlines()
...
def weighted_eval(
    ...
    META_PATH = os.path.expanduser("~")+'/H-drive/srv/studat/cdt/team2'+'/data/legasee/metadata',
    # Set to "train" if gold transcript data stored in train folder.
    TEST_TRAIN = 'plain_text',
    ...
    if SUB_FOLDERS:
        ...
        for _fn in os.scandir(M_FOLDER):
    ...
    # Output results
    scores_df.to_csv(save_folder+'/'+'evaluation_scores.tsv',sep='\t')
    ...
    wil = 1 - scores_df['Unweighted','wip'].mean()
    wwil = 1 - scores_df['Weighted (own + shared keywords)','wip'].mean()

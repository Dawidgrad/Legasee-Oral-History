...
def kenlm_decoder(arpa, vocab, alpha=0.45, beta=0.8):  
    alpha = alpha
    beta = beta
...
def decode_lm(logits, decoder):
    ...
    with multiprocessing.get_context('fork').Pool() as pool:
        decoded = decoder.decode_batch(pool, logits, beam_width=200)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, help='Path to model file', default='../train/just_legasee')
    ...
    parser.add_argument('--output_dir', type=str, help='Path to output directory (NONE if saving of outputs is not needed)', default='wav2vec2_justlegasee_lm')
